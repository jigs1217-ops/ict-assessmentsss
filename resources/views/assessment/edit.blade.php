...
<div class="container-fluid">
    <div class="page-header mb-4 border-bottom pb-2">
        ...
        <p class="text-muted">Update any field of the assessment.</p>
    </div>
                ...
                <div class="card-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6" id="division-container">
                            ...
                            @if ($assessment->division && isset($deptSecList[$assessment->department]) && in_array($assessment->division, $deptSecList[$assessment->department]))
                                <div class="form-control bg-light">{{ $assessment->division }}</div>
                                <input type="hidden" name="division" value="{{ $assessment->division }}">
                            @else
                                <div class="form-control bg-light">Select a department first</div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select name="equipment_type" id="equipment_type" class="form-select" required>
                            ...
                            <option value="Desktop Computer" {{ old('equipment_type', $assessment->equipment_type) == 'Desktop Computer' ? 'selected' : '' }}>Desktop Computer</option>
                            <option value="Laptop Computer" {{ old('equipment_type', $assessment->equipment_type) == 'Laptop Computer' ? 'selected' : '' }}>Laptop Computer</option>
                            <option value="Network Printer / All-in-one Printer" {{ old('equipment_type', $assessment->equipment_type) == 'Network Printer / All-in-one Printer' ? 'selected' : '' }}>Network Printer / All-in-one Printer</option>
                            <option value="Tablet / IPad" {{ old('equipment_type', $assessment->equipment_type) == 'Tablet / IPad' ? 'selected' : '' }}>Tablet / iPad</option>
                            <option value="Server Computer" {{ old('equipment_type', $assessment->equipment_type) == 'Server Computer' ? 'selected' : '' }}>Server Computer</option>
                        </select>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateDivisions() {
        ...
        select.innerHTML = '<option value="">Select Division</option>' +
            divisions.map(d => `<option value="${d}" ${d == "{{ old('division', $assessment->division) }}" ? 'selected' : ''}>${d}</option>`).join('');
    ...
    function updateFields() {
        ...
        // Pre-fill values for existing assessment
        if (selected) {
            const fieldsToFill = ['model', 'date_acquired', 'date_assessed', 'motherboard', 'processor', 'memory', 'harddisk_capacity', 'os', 'ms_office'];
            fieldsToFill.forEach(field => {
                const input = document.getElementById(field);
                if (input) {
                    input.value = "{{ old($field, $assessment->$field) }}";
                }
            });

            ['lan_connected', 'internet_connected'].forEach(field => {
                const select = document.getElementById(field);
                if (select) {
                    select.value = "{{ old($field, $assessment->$field) }}";
                }
            });
        }
    }
    ...
    function populateAnalysis(cond) {
        ...
        analysisOutput.innerHTML = `<select name="analysis" class="form-select" required>
            <option value="">Select</option>
            ${analysisOptions.map(a => `<option value="${a.value}" ${a.value == "{{ old('analysis', $assessment->analysis) }}" ? 'selected' : ''}>${a.label}</option>`).join('')}
        </select>`;
        ...
        if (analysisOptions.length === 1) {
            ...
            // Set value from old or assessment
            analysisSelect.value = "{{ old('analysis', $assessment->analysis) }}";
            hiddenAnalysisInput.value = "{{ old('analysis', $assessment->analysis) }}";
        }

        // Reset recommendation dropdown
        populateRecommendation(cond, analysisSelect.value);
    ...
    function populateRecommendation(cond, analysisVal) {
        ...
        recommendationOutput.innerHTML = `<select name="recommendation" class="form-select" required>
            <option value="">Select</option>
            ${recOptions.map(r => `<option value="${r.value}" ${r.value == "{{ old('recommendation', $assessment->recommendation) }}" ? 'selected' : ''}>${r.label}</option>`).join('')}
        </select>`;
        ...
        if (recOptions.length === 1) {
            ...
            // Set value from old or assessment
            recommendationSelect.value = "{{ old('recommendation', $assessment->recommendation) }}";
            hiddenRecommendationInput.value = "{{ old('recommendation', $assessment->recommendation) }}";
        }
        ...
        form.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Since this is an edit, you might want to redirect or show a success message differently.
                    // For now, we'll show a simple alert or redirect.
                    alert('Assessment updated successfully!');
                    window.location.href = "{{ route('assessment.index') }}"; // Redirect to index
                } else {
